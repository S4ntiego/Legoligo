...
;(function () {
      ...
      for (prop in under) {
        if (under.hasOwnProperty(prop)) {
          ...
          if (typeof data[prop] === 'undefined') {
            if (type === 'object') {
              fn.rebase(
                (data[prop] = fn.isArray(under[prop]) ? [] : {}),
                under[prop]
              )
            } else {
    ...
    switch (options.src) {
      case 'sprite':
        ...
        if (options.wrap > 0) {
          pre =
            'height:' +
            getImgSize(img.h, options.size) +
            'px; width:' +
            getImgSize(img.w, options.size) +
            "px; background: url('"
          post =
            "') -" +
            getImgSize(img.x, options.size) +
            'px -' +
            getImgSize(img.y, options.size) +
            'px no-repeat;'
          if (options.wrap > 1) {
  ...
  fn.fire = {
    ...
    plan: function (group, planData, callback) {
      ...
      return (
        fn.fire.data[group].push({ planData: planData, callback: callback }) - 1
      )
    },
    /**
     * @method
    ...
    exec: function (group, execData) {
      if (typeof fn.fire.data[group] === 'object') {
        ...
        for (i = 0; i < fn.fire.data[group].length; i++) {
          fn.fire.data[group][i].callback(
            fn.fire.data[group][i].planData,
            execData,
            i
          )
        }
...
Riot.DDragon.fn.init = function (selectorEngine) {
    ...
    champion: [
      function (obj) {
        if (typeof obj.loadedPartial === 'undefined') {
          ...
          obj.stats.attackspeed = fn.calcAspd(
            1,
            obj.stats.attackspeedoffset,
            obj.stats.attackspeedperlevel
          )
      ...
      if (typeof obj.name === 'string') {
        ...
        if (obj.tags) {
          for (i = 0, j = obj.tags.length; i < j; ++i) {
            ...
            obj.searchString +=
              ';' + fn.tFilter(fn.t(obj.tags[i])) + translatedColloqTag
          }
        }
      }
    }
      ...
      if (typeof options !== 'object') {
        ...
        if (options.wrap) {
          if (options.wrap > 2) {
            ...
            options.attrs +=
              ' data-rg-name="' + this.name + '" data-rg-id="' + id + '"'
          }
        }
        options = fn.rebase(options, basic)
      }
      ...
      // This is a much more language aware and dynamic search than standard
        ...
        if ((fm = /^searchKey:(.*)$/.exec(this.fData[fi]))) {
          ...
          for (fi_ = 0, fj_ = fm.length; fi_ < fj_; ++fi_) {
            ...
            this.fData[ftemp === fi_ ? fi : this.fData.length] = fn.makeTest(
              'searchString*=' + fm[fi_]
            )
          }
  ...
  dd.model.prototype.remodel = function (newModel, newKeys) {
    ...
    if (this.init) {
      return (dd.models[newModel] = new dd.model(
        this.type,
        this.version,
        JSON.stringify({
        ...
        switch (action._action) {
          case 'collect':
            if (action.s && action.f)
              return model.collect(action.fData, action.sData)
    ...
    if (info && info.aux) {
      dd.addApp([info.name], function () {
        ...
        valueAux('img', function (value) {
          el.innerHTML += dd
            .useModel(info.name)
            .getImg(info.id, { src: value, wrap: info.wrap, size: info.size })
        })

        valueAux('query', function (value) {
          var obj = dd.useModel(info.name).get(info.id)
          if (!obj.hasOwnProperty('_action')) {
            el.innerHTML += fn.objectDig(
              dd.useModel(info.name).get(info.id),
              value
            )
          }
        })
 ...
 */
  ...
  dd.addDisplay({
    ...
    success: function (data) {
      ...
      return (
        ...
        this.applyDisplay('item_tree_worker', data) +
        '</div></div>'
      )
    },
  })
  ...
  dd.addDisplay({
    ...
    success: function (data) {
      ...
      r = this.model.getImg(data.id, {
        ...
        wrap: 3,
        classes:
          this.runClassTest(data) + (select ? 'selected ' : '') + 'treeimg',
      })

      // Begin tree building if there are children
      if (j !== 0) {
        ;(width = 100 / j), (spread = j === 1 ? 'single' : 'multi')
        ...
        for (i = 0; i < j; i++) {
          ...
          r +=
            ...
            spread +
            '"></div>' +
            self.call(this, this.model.get(data.from[i])) +
            '</div>'
        }
      }
      return r
  ...
  dd.addDisplay({
    ...
    success: function (data) {
      return (
        ...
        fn.t('Cost_') +
        ' <span class="gold">' +
        (data.specialRecipe !== 0
          ? fn.t('SpecialRecipeLarge')
          : data.gold.total + ' (' + data.gold.base + ')') +
        '</span></div>' +
        '</div>'
      )
    },
  ...
  dd.addDisplay({
    ...
    success: function (data) {
      return (
        '' +
        '<div class="item-into">' +
        this.applyDisplay('item_modal_build', data) +
        '</div>' +
        this.applyDisplay('item_tree', data) +
        '<div class="item-desc">' +
        this.applyDisplay('item_modal_description', data) +
        '</div>'
      )
    },
    onFill: function () {
      ...
      fn.addEvent(this.box, 'mousedown', function (data) {
        ...
        if (d) {
          ...
          if (fn.hasClass(data.target, 'treeimg')) {
            ...
            fn.$('.item-desc', this.box)[0].innerHTML = that.view.applyDisplay(
              'item_modal_description',
              item
            )
            fn.$('.item-into', this.box)[0].innerHTML = that.view.applyDisplay(
              'item_modal_build',
              item
            )
    ...
    try {
      ...
      if (typeof dd.display[display] === 'undefined') {
        r = fn.er(
          e,
          'view.applyDisplay',
          'Nonexistent display (' + display + ')'
        )
      } else {
        r = fn.er(
          e,
          'view.applyDisplay',
          'Invalid data used for display (' + display + ')'
        )
      }
    }
    return r
  ...
  dd.view.prototype.reload = function () {
    if (
      !this.init ||
      (typeof data === 'object' && typeof data._action === 'string')
    ) {
 ...
 */
  ...
  dd.controller.prototype.loadBox = function (type) {
    ...
    if (
      ...
      this.boxData.dataTarget === 'commonbox'
    ) {
    ...
    if (this.box !== false && this.view) {
      ...
      if (this.view.init === false) {
        fn.fire.plan(
          ...
          function (p, e, fireId) {
            if (
              typeof p.view.data !== 'undefined' &&
              p.view.data._name === e.name
            ) {
              p.view.reload()
  ...
  dd.controller.prototype.addEvent = function (event, callback) {
    var self = this
    fn.addEvent(
      this.target || fn.$(this.boxData.target)[0],
      event,
      function (e) {
  ...
  dd.controller.prototype.reactive = function (data) {
    ...
    if (data.buttonRestrict !== -1 && data.button !== data.buttonRestrict)
      return
  ...
  dd.player.prototype.updateMasteryStringFor = function (id) {
    ...
    this.masteryString =
      this.masteryString.slice(0, pos) +
      this.mastery[id] +
      this.masteryString.slice(pos + 1)
  }
  ...
  fn.calcAspd = function (level, offset, aspdper, aspd) {
    aspd = aspd || 0
    var i =
      (1 / (1.6 * (1 + offset))) * (1 + aspd + (aspdper * (level - 1)) / 100)
